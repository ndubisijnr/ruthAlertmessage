  ...
  <div class="on_boarding_input">
      <div :style="info ? {width:'100%',display: 'flex',alignContent: 'center',position:'relative'} : null">
        ...
        <div style="position: relative;width: 100%">
          ...
          <div>
            <input :id="id" :autocomplete="autocomplete" :max="max" required  :style="width ? {width:width} :  null"  :type="type" class="formInput" :placeholder="placeholder"  :class="{'focused':isFocused}" :readonly="readonly" v-model="inputValue" @focus="handleFocus" @focusout="handleFocusOut" @input="emitValue" />
            <div class="loader">
...
export default {
  ...
  props:['type', 'value','max', 'isFakeLoading', 'autocomplete', 'className','label','width','id', 'error','required', 'errorInput', 'placeholder', 'readonly', 'info'],
  data(){
    return{
      inputValue:null,
      isFocused:false,
