<template>
  <div class="on_boarding_input">
    ...
    <input :id="id" :style="width ? {width:width} :  null "  :type="type" class="formInput"  :class="{'focused':isFocused}" v-model="inputValue" @focus="handleFocus" @focusout="handleFocusOut" @input="emitValue" required="true"/>
...
export default {
  ...
  props:['type', 'className','label','width','id', 'error','required'],
  data(){
    return{
      inputValue:null,
      isFocused:false
