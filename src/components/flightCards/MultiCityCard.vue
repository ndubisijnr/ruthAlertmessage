...
<!--            {{j.segments.length}}-->
            <section style="height: 7.125rem;display: flex;align-items: center" @click="showDetails(index)" v-for="(x, indexx) in j.segments" :key="indexx">
              <div class="logo-area">
              ...
              <div class="actual-result-item-info">
                <div>
                  ...
                  <p class="dest">{{ getCityByCityCode(x.airport_from) }}
                    ({{ x.airport_from }})</p>
                </div>
                <div class="more-flight-info">
                  <span class="duration">{{ convertDurationToWords(j.total_segment_duration) }}</span>
                  ...
                  <span class="stops">{{ j.total_segment_stops === 0 ? 'Direct' :
                      `${j.total_segment_stops}-Stop`}}</span>
                </div>
                <div>
      ...
      <div class="amount-book-area"
           :style="i.routes.length > 0 ? { 'display': 'flex !important', 'align-items': 'center !important', 'height': '15.5rem !important' } : null">
        <div style="width: 100%;">
          ...
          <div >
            <div id="departure" class="dropdown-details-div">
              <div class="depart_date-info">
                <p class="depart-date">Depart Flight {{getCityByCityCode(i.routes[0].segments[0]?.airport_from) }} - {{ getCityByCityCode(i.routes[0].segments[0]?.airport_to) }}</p>
              </div>
...
<!--              </svg>-->
            ...
            <div v-for="f in i.routes">
                <div v-for="j in f.segments">
