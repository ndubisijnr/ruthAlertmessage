<template>
    <div :class="{ break: items.break }" class="flex_between">
        <label :for="items.id">
            {{ items.label }}
            <span v-if="items.required || items.showReq" class="text-danger"
                >*</span
            >
        </label>
        <div class="d-flex align-center relative">
            ...
            >
                ...
                <div
                    @click.stop="
                        $emit('changeMenu', loopedId + objectKey + '00'),
                            (items.inputType.openVal =
                                loopedId + objectKey + '00')
                    "
                    :class="{
                    ...
                    <template v-else-if="items.inputType.type === 'cities'">
                        <div class="airlines_wrapper d-flex flex-column h-100">
                            <template v-if="items.mapVal.length">
                                ...
                                <div
                                    v-if="
                                        items.inputType.openVal === currentMenu
                                    "
                                    class="checkWapper d-flex flex-column h-100"
                                >
                                    <div
    ...
    methods: {
        ...
        simpleLowerCheck(item) {
            ...
            if (this.mainObj !== "general") {
                ...
                if (
                    this.formData[this.product][this.mainObj][
                        this.loopedId
                    ].val?.includes(item)
                ) {
                    val = true;
            ...
            switch (type) {
                default:
                    if (
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].val?.includes(item.code)
                    ) {
                        val = true;
                    }
                    break;
            ...
            switch (type) {
                case "cities":
                    if (this.lowerCheck(type, item).val) {
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].val = arrw.val.filter((el) => {
                            return el !== item.code;
                        });
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].userVal = arrw.userVal.filter((el) => {
                            return el !== item.city;
                        });
                    } else {
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].val.push(item.code);
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].userVal.push(item.city);
                    }
                    break;
                default:
                    if (this.lowerCheck(type, item).val) {
                        ...
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].userVal.push(item.name);
                    }
        ...
        setSimpleCheck(item) {
            if (this.mainObj !== "general") {
                ...
                const arrw =
                    this.formData[this.product][this.mainObj][this.loopedId];
                if (this.simpleLowerCheck(item).val) {
                    this.formData[this.product][this.mainObj][
                        this.loopedId
                    ].val = arrw.val.filter((el) => {
            ...
            switch (type) {
                case "airlines":
                    if (arrw.length && arrw.length < this.airlines.length) {
                        ...
                        arrw.length &&
                        arrw.length === this.airlines.length
                    ) {
                        val = "checked";
                    } else {
                        val = null;
            ...
            if (
                ...
                switch (type) {
                    case "airlines":
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].val = this.airlines.map((el) => {
                            return el.code;
                        });
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].userVal = this.airlines.map((el) => {
                            return el.name;
                        });
                    ...
                    case "countries":
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].val = countries.map((el) => {
                            return el.code;
                        });
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].userVal = countries.map((el) => {
                            return el.name;
                        });
                        break;
                    case "cities":
                        ...
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].val = mapVal.map((el) => {
                            return el.code;
                        });
                        this.formData[this.product][this.mainObj][
                            this.objectKey
                        ][this.loopedId].userVal = mapVal.map((el) => {
                            return el.city;
                        });
                        break;
        ...
        async getCities(val) {
            ...
            try {
                ...
                this.formData[this.product][this.mainObj][this.objectKey][
                    this.loopedId + 2
                ].mapVal = this.replaceKeys(
                    this.removeDuplicates(res, "city_code"),
