<template>
  <div :class="{ break: items.break }" class="flex_between">
    <label :for="items.id">
      {{ items.label }}
      <span v-if="items.required || items.showReq" class="text-danger">*</span>
    </label>
    <div class="d-flex align-center relative">
      ...
      <div
        ...
        "
        v-else-if="items.inputType.structure === 'select'"
      ...
      <ContentInput
        v-else-if="items.inputType.structure === 'content-input'"
        :initialPlaceholder="items.label"
        @contentEmit="items.val = $event"
        :initialContent="items.val"
      />
      <img
        class="absolute"
        v-if="items.tooltip"
