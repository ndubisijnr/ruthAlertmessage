      ...
      <tr v-for="h in fields" :key="h.key" class="table-cell">
        ...
        >
          <td @click="table_row_onclick_action(j)" class="table-row px-6 py-3">
            ...
            <span
              class="connected"
              :class="{ pending: j.status_m !== 1 }"
              v-if="h.key === 'status_m'"
              >{{ j.status_m === 1 ? "Active " : "Inactive" }}</span
            >
            <!-- template {domain status}  -->
            <span
              class="connected"
              :class="{ pending: j.active !== 1 }"
              v-else-if="h.label.toLowerCase() === 'status'"
              >{{ j.active === 1 ? "Connected " : "Pending" }}</span
            >

            <span
              v-else-if="h.label === 'Member Status'"
              :style="
                j.status === 'pending'
                  ? { color: '#F1D302' }
                  : j.status === 'active'
            ...
            <span
              v-else-if="
                h.label.toLowerCase() === 'domain name' && j.is_primary === 1
              "
            >
              {{ j.domain }}
              <svg
              ...
              >
                <path
                  d="M15 8.25C14.8011 8.25 14.6103 8.32902 14.4697 8.46967C14.329 8.61032 14.25 8.80109 14.25 9V13.5C14.25 13.6989 14.171 13.8897 14.0303 14.0303C13.8897 14.171 13.6989 14.25 13.5 14.25H4.5C4.30109 14.25 4.11032 14.171 3.96967 14.0303C3.82902 13.8897 3.75 13.6989 3.75 13.5V4.5C3.75 4.30109 3.82902 4.11032 3.96967 3.96967C4.11032 3.82902 4.30109 3.75 4.5 3.75H9C9.19891 3.75 9.38968 3.67098 9.53033 3.53033C9.67098 3.38968 9.75 3.19891 9.75 3C9.75 2.80109 9.67098 2.61032 9.53033 2.46967C9.38968 2.32902 9.19891 2.25 9 2.25H4.5C3.90326 2.25 3.33097 2.48705 2.90901 2.90901C2.48705 3.33097 2.25 3.90326 2.25 4.5V13.5C2.25 14.0967 2.48705 14.669 2.90901 15.091C3.33097 15.5129 3.90326 15.75 4.5 15.75H13.5C14.0967 15.75 14.669 15.5129 15.091 15.091C15.5129 14.669 15.75 14.0967 15.75 13.5V9C15.75 8.80109 15.671 8.61032 15.5303 8.46967C15.3897 8.32902 15.1989 8.25 15 8.25Z"
                  fill="black"
                />
                <path
                  d="M12.0004 3.75H13.1854L8.46794 8.46C8.39765 8.52972 8.34185 8.61267 8.30377 8.70407C8.2657 8.79546 8.24609 8.89349 8.24609 8.9925C8.24609 9.09151 8.2657 9.18954 8.30377 9.28093C8.34185 9.37233 8.39765 9.45528 8.46794 9.525C8.53766 9.5953 8.62062 9.65109 8.71201 9.68917C8.8034 9.72724 8.90143 9.74685 9.00044 9.74685C9.09945 9.74685 9.19748 9.72724 9.28888 9.68917C9.38027 9.65109 9.46322 9.5953 9.53294 9.525L14.2504 4.815V6C14.2504 6.19891 14.3295 6.38968 14.4701 6.53033C14.6108 6.67098 14.8015 6.75 15.0004 6.75C15.1994 6.75 15.3901 6.67098 15.5308 6.53033C15.6714 6.38968 15.7504 6.19891 15.7504 6V3C15.7504 2.80109 15.6714 2.61032 15.5308 2.46967C15.3901 2.32902 15.1994 2.25 15.0004 2.25H12.0004C11.8015 2.25 11.6108 2.32902 11.4701 2.46967C11.3295 2.61032 11.2504 2.80109 11.2504 3C11.2504 3.19891 11.3295 3.38968 11.4701 3.53033C11.6108 3.67098 11.8015 3.75 12.0004 3.75Z"
                  fill="black"
                />
              </svg>
              <svg
            ...
            <div v-else-if="h.label.toLowerCase() === 'action'">
              <span
                @click="(currentActionIndex = index), (show = !show)"
                :style="
                  j.type === 'manager'
                    ? { cursor: 'not-allowed' }
                    : { cursor: 'pointer' }
              ...
              <div class="menu" v-show="currentActionIndex === index && show">
                <!-- template {team} -->
                <div v-if="h.id === 'member'" v-show="j.type !== 'manager'">
                  <p class="menu-item" @click="editTeamMember(j)">
                    Edit Member
                  </p>
                  <p
                ...
                <div v-if="h.id === 'team'">
                  ...
                  <p class="menu-item deactivate" @click="confirmDeactiveAgent">
                    Deactivate Agent
                  </p>
                </div>
            ...
            <span v-else-if="h.label.toLowerCase() === 'name'"
            ...
            <span
              v-else-if="h.label === 'Customer’s Name'"
              style="text-transform: capitalize"
            >
              {{ j.flight.passengers[0].first_name }}
            ...
            <span v-else-if="h.label === 'Airline'"
              ><img
                width="20"
                :src="j.flight?.outbound[0]?.airline_details.logo"
                alt="flight_logo"
              />
              {{ j.flight?.outbound[0]?.airline_details.name }}</span
            >

            <!-- template {bookings status} -->

            <span
              v-else-if="h.label === 'Status_'"
              :style="
                j.status === 'reserved'
                  ? { 'text-transform': 'Capitalized', color: '#F1D302' }
                  : j.status === 'issued'
                  ? { 'text-transform': 'Capitalized', color: '#159D54' }
                  : { 'text-transform': 'Capitalized', color: '#F04444' }
              "
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
            ...
            <span v-else-if="h.label.toLowerCase() === 'ticket amount'"
              >₦ {{ formatAmount(j?.amount) }}</span
            >

            <!-- template {agent balance}  -->

            <span v-else-if="h.label.toLowerCase() === 'wallet balance'"
              >₦ {{ j.wallet.balance }}</span
            >

            <!-- template {roles number}  -->
            <span v-else-if="h.label.toLowerCase() === 'no.of member'">{{
              j?.users?.length
            }}</span>
            ...
            <span v-else-if="h.key === 'is_corporate'">{{
              j.is_corporate === "true" ? "Coorporate" : "Personal"
            }}</span>

            <!-- template {permission number}  -->
            <span v-else-if="h.label.toLowerCase() === 'no.of permission'">{{
  ...
  methods: {
    ...
    table_row_onclick_action(obj) {
      ...
      if (this.getCurrentRoute.toLowerCase() === "bookings") {
        ...
        router.push({
          path: `/bookings/details/${this.getUser?.access_token?.slice(0, 20)}`,
        });
      }
    },
