...
<on-boarding-button :loading="deleteLoading" :disabled="deleteLoading" @click="deleteBankAction"
btn-width="11.0625rem" border="none" background="#F04444" text-node="Yes, Delete"></on-boarding-button>
</div>
</div>
...
<p class="are-you-sure-p">
You cannot delete this bank account information because you
have just one bank account saved in your payment method.
Please add a new bank account to replace this card so you
can delete this card information..
</p>
</div>
</div>
</div>
...
<div class="tab-nav">
<ul class="inner-tab-nav" :style="getBusinessProfile?.is_cac_verified ===
'true' &&
...
<a v-if="getUser?.account_type === 'super_admin' ||
  (getUser?.account_type === 'admin' &&
    getBusinessProfile?.is_cac_verified ===
    'true' &&
    getBusinessProfile?.is_id_verified ===
    'true')
  " :class="{ active: currentTab === 'Domain' }" href="#Domain" @click="activateTab('Domain')">Domain</a>
<a v-if="getUser?.account_type === 'super_admin' ||
  getUser?.account_type === 'admin' ||
  (getBusinessProfile?.is_cac_verified ===
    'true' &&
    getBusinessProfile?.is_id_verified ===
    'true')
  " :class="{ active: currentTab === 'Teams' }" @click="activateTab('Teams')" href="#Teams">Teams</a>
<a :class="{
  active: currentTab === 'Notifications',
}" v-if="getUser?.account_type === 'super_admin' ||
getUser?.account_type === 'admin' ||
(getBusinessProfile?.is_cac_verified ===
'true' &&
getBusinessProfile?.is_id_verified ===
'true')
" href="#Notifications" @click="activateTab('Notifications')">Notifications</a>
<a v-if="getUser?.account_type === 'super_admin' ||
  getUser?.account_type === 'admin' ||
  (getUser?.account_type !== 'booker' &&
    getBusinessProfile?.is_cac_verified ===
    'true' &&
    getBusinessProfile?.is_id_verified ===
    'true')
  " :class="{ active: currentTab === 'Payment' }" href="#Payment" @click="activateTab('Payment')">Payment
</a>
<a :class="{ active: currentTab === 'Markup' }" v-if="getUser?.account_type === 'super_admin' ||
  getUser?.account_type === 'admin' ||
  (getBusinessProfile?.is_cac_verified ===
    'true' &&
    getBusinessProfile?.is_id_verified ===
    'true')
  " href="#Markup" @click="activateTab('Markup')">Markup</a>
<a :class="{
  active: currentTab === 'Customization',
}" href="#Customization" @click="
activateTab('Customization'),
(isEditing = true)
">Customization</a>
</ul>
</div>
      ...
      <form>
        <div class="grouped_input">
          <div class="on_boarding_input">
            ...
            <input id="first_name" :class="{
              focused: isFocused,
            }" v-model="model1.first_name
" @focus="handleFocus" @focusout="handleFocusOut
" type="email" class="formInput" />
          </div>
          <div class="on_boarding_input">
            <label class="label" :class="{
              focused: isFocused,
            }">Last Name</label>
            <input id="last_name" v-model="model1.last_name
              " :class="{
focused: isFocused,
}" @focus="handleFocus" @focusout="handleFocusOut
" type="email" class="formInput" />
        ...
        <div class="on_boarding_input">
          ...
          <input id="email" :class="{
            focused: isFocused,
          }" v-model="model1.email" @focus="handleFocus" @focusout="handleFocusOut" type="email"
        ...
        <div class="on_boarding_input">
          ...
          <input id="phone" :class="{
            focused: isFocused,
          }" v-model="model1.phone" @focus="handleFocus" @focusout="handleFocusOut" type="email"
            class="formInput" />
        </div>
      </form>
      <div class="change_password">
        <div>
          ...
          <p class="change_password-sub">
            You can change your password
            here!
          </p>
        </div>
        <on-boarding-button @click="changePassword = true" text-node="Change Password" btn-width="10rem"
          height="3rem" :background="lightenColor(
            custom_theme
              ? custom_theme.color
              : default_theme?.color
          )
            " border="none"></on-boarding-button>
      </div>
      <div style="
                                  display: flex;
                                  justify-content: end;
                              ">
        <OnBoardingButton @click="handleUpdateProfile" :loading="loading" :disabled="loading" border="none"
          :textNode="'Save Changes'" :btnWidth="'11.0625rem'"></OnBoardingButton>
      </div>
    </div>
  </div>
</div>
...
<div class="domain-header">
  <div>
    ...
    <p class="p-sub-domain">
      By default, your site is always
      accessible via a Tiqwa subdomain based
      on the site name. Custom domains allow
      you to access your site via one or more
      custom domain names.
    </p>
  </div>

  <on-boarding-button @click="addDomain = true" fontsize="1rem" color="#FFFFFF"
    text-node="Add Custom Domain" btn-width="12.25rem" height="2.5rem" border="none"></on-boarding-button>
</div>
...
<div class="teams-header">
  <div class="search-team">
    ...
    <input v-model="searchQuery" type="search" @input="doFilter" style="
                              outline: none;
                              border: none;
                              width: 19.4rem;
  ...
  <div class="btn3">
    <on-boarding-button v-show="activeManageRole === 'team'" class="role" @click="addMember = true"
      btn-width="10.625rem" height="2.5rem" text-node="Add New Member" fontsize="12px"></on-boarding-button>
    <on-boarding-button v-show="activeManageRole === 'permissions'
      " class="role" @click="addRole = true" btn-width="10.625rem" height="2.5rem"
      text-node="Create role" fontsize="12px"></on-boarding-button>
  </div>
</div>
...
<div class="teams-main" v-if="!noUser">
  <div v-if="activeManageRole === 'team'">
    <div v-if="getMembers?.length > 0" class="table-wrapper">
      <domain-table @updatingTeamMember="updateTeam" @deactivatingTeamMember="deactivateTeamMember
        " :is-paginate="true" :data="membersFilteredResult.length > 0
? membersFilteredResult
: getMembers
" :fields="membersFields"></domain-table>
    </div>
    <div v-else class="no-team-member">
      <svg xmlns="http://www.w3.org/2000/svg" width="116" height="116" viewBox="0 0 116 116" fill="none">
        <path
          ...
          fill="#F1F2F6" />
        <path d="M8.3418 20.9214L67.758 3.4748L90.5149 80.9757L31.0986 98.4223L8.3418 20.9214Z" fill="white"
          stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M20.0869 8.62842H82.0131V89.3999H20.0869V8.62842Z" fill="white" stroke="#1D1E2C"
          stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M28.166 16.707H73.937V24.7846H28.166V16.707Z" fill="#E5E9F2" stroke="#1D1E2C"
          stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M44.3193 38.2441H55.0899" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path d="M44.3193 43.6309H68.5521" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path d="M44.3193 49.0151H61.8405" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path d="M44.3193 65.1689H57.7815" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path d="M44.3193 70.5532H71.2436" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path d="M44.3193 75.9399H65.8591" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path
        ...
        <path
          d="M110.051 101.615C110.527 102.079 110.906 102.632 111.166 103.244C111.427 103.856 111.563 104.513 111.567 105.177C111.572 105.842 111.444 106.501 111.192 107.116C110.939 107.731 110.567 108.289 110.097 108.759C109.627 109.23 109.069 109.602 108.454 109.854C107.839 110.106 107.18 110.234 106.516 110.23C105.851 110.226 105.194 110.09 104.582 109.829C103.971 109.569 103.417 109.19 102.953 108.714L89.6396 95.4011L96.739 88.3018L110.051 101.615Z"
          stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
        <path d="M82.5391 84.7549L86.0882 81.2048L94.964 90.0781L91.4148 93.6282L82.5391 84.7549Z"
          fill="white" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
          stroke-linejoin="round" />
        <path
  ...
  <div v-else>
    <div v-if="getRoles?.length > 0" class="table-wrapper">
      <domain-table @updatingRole="editRole" :is-paginate="true" :data="getRoles"
        :fields="rolesFields"></domain-table>
    </div>
    <div v-else class="no-team-member">
      <svg xmlns="http://www.w3.org/2000/svg" width="116" height="116" viewBox="0 0 116 116" fill="none">
        <path
...
<div v-else class="no-team-member">
  <svg xmlns="http://www.w3.org/2000/svg" width="116" height="116" viewBox="0 0 116 116" fill="none">
    ...
    <path
      d="M110.051 101.615C110.527 102.079 110.906 102.632 111.166 103.244C111.427 103.856 111.563 104.513 111.567 105.177C111.572 105.842 111.444 106.501 111.192 107.116C110.939 107.731 110.567 108.289 110.097 108.759C109.627 109.23 109.069 109.602 108.454 109.854C107.839 110.106 107.18 110.234 106.516 110.23C105.851 110.226 105.194 110.09 104.582 109.829C103.971 109.569 103.417 109.19 102.953 108.714L89.6396 95.4011L96.739 88.3018L110.051 101.615Z"
      stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
    <path d="M82.5391 84.7549L86.0882 81.2048L94.964 90.0781L91.4148 93.6282L82.5391 84.7549Z" fill="white"
      stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
    <path
      d="M86.9799 53.6947C90.1393 56.854 92.2908 60.8792 93.1625 65.2613C94.0342 69.6434 93.5869 74.1856 91.8771 78.3135C90.1673 82.4414 87.2719 85.9695 83.5569 88.4518C79.8419 90.9341 75.4743 92.259 71.0063 92.259C66.5384 92.259 62.1707 90.9341 58.4558 88.4518C54.7408 85.9695 51.8453 82.4414 50.1356 78.3135C48.4258 74.1856 47.9784 69.6434 48.8501 65.2613C49.7218 60.8792 51.8734 56.854 55.0328 53.6947C59.2696 49.4591 65.0154 47.0796 71.0063 47.0796C76.9973 47.0796 82.743 49.4591 86.9799 53.6947Z"
      fill="white" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
    <path
      d="M82.5389 58.1331C84.8205 60.4147 86.3742 63.3216 87.0037 66.4863C87.6332 69.6509 87.3101 72.9312 86.0753 75.9122C84.8405 78.8933 82.7494 81.4412 80.0666 83.2338C77.3837 85.0265 74.2295 85.9833 71.0028 85.9833C67.7762 85.9833 64.622 85.0265 61.9391 83.2338C59.2562 81.4412 57.1652 78.8933 55.9304 75.9122C54.6956 72.9312 54.3725 69.6509 55.0019 66.4863C55.6314 63.3216 57.1852 60.4147 59.4667 58.1331C60.9816 56.6179 62.78 55.4161 64.7594 54.5961C66.7388 53.7761 68.8603 53.354 71.0028 53.354C73.1453 53.354 75.2669 53.7761 77.2462 54.5961C79.2256 55.4161 81.0241 56.6179 82.5389 58.1331Z"
      fill="#E5E9F2" />
    <path
...
<!-- notifications -->
<div id="notifications" class="notifications animate__animated animate__fadeIn"
...
<div class="notification-wrapper">
  <div class="notification-inner">
    <div>
      ...
      <p class="n-2">
        These are general notifications
        about updates and goings-on on the
        platform
      </p>
    </div>
    ...
    <div class="notification-items">
      <div class="item">
        ...
        <img style="margin-right: -10px" src="../../assets/Switchon.svg"
          @click="(getNotifications.message_in_app_notification = 0), updateNotification()"
          v-if="getNotifications?.message_in_app_notification" />
        <img src="../../assets/Switchoff.svg" @click="
          (getNotifications.message_in_app_notification = 1),
          updateNotification()
          " v-else />
      </div>
      <div class="item">
        <p class="n-3">Send to email</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.message_email_notification = 0),
          updateNotification()" v-if="getNotifications?.message_email_notification" />
        <img src="../../assets/Switchoff.svg" @click="
          (getNotifications.message_email_notification = 1),
          updateNotification()
          " v-else />
      </div>
      <div class="item">
        <p class="n-3">Text to phone</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="
          (getNotifications.message_sms_notification = 0),
          updateNotification()
          " v-if="getNotifications?.message_sms_notification
" />
        <img src="../../assets/Switchoff.svg" @click="
          (getNotifications.message_sms_notification = 1),
          updateNotification()
          " v-else />
      </div>
    </div>
  ...
  <div class="notification-inner">
    ...
    <div class="notification-items">
      <div class="item">
        <p class="n-3">In-app</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.reminder_in_app_notification = 0),
          updateNotification()" v-if="getNotifications?.reminder_in_app_notification" />
        <img src="../../assets/Switchoff.svg" @click="
          (getNotifications.reminder_in_app_notification = 1),
          updateNotification()
          " v-else />
      </div>
      <div class="item">
        <p class="n-3">Send to email</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.reminder_email_notification = 0),
          updateNotification()" v-if="getNotifications?.reminder_email_notification" />
        <img src="../../assets/Switchoff.svg" @click="
          (getNotifications.reminder_email_notification = 1),
          updateNotification()
          " v-else />
      </div>
      <div class="item">
        <p class="n-3">Text to phone</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.reminder_sms_notification = 0),
          updateNotification()" v-if="getNotifications?.reminder_sms_notification" />
        <img src="../../assets/Switchoff.svg" @click="
          (getNotifications.reminder_sms_notification = 1),
          updateNotification()
          " v-else />
      </div>
    </div>
  ...
  <div class="notification-inner">
    ...
    <div class="notification-items">
      <div class="item">
        <p class="n-3">In-app</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.ads_newsletter_in_app_notification = 0),
          updateNotification()" v-if="getNotifications?.ads_newsletter_in_app_notification" />
        <img src="../../assets/Switchoff.svg" @click="(getNotifications.ads_newsletter_in_app_notification = 1),
          updateNotification()" v-else />
      </div>
      <div class="item">
        <p class="n-3">Send to email</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.ads_newsletter_email_notification = 0),
          updateNotification()" v-if="getNotifications?.ads_newsletter_email_notification" />
        <img src="../../assets/Switchoff.svg" @click="(getNotifications.ads_newsletter_email_notification = 1),
          updateNotification()" v-else />
      </div>
      <div class="item">
        <p class="n-3">Text to phone</p>
        <img style="margin-right: -10px" src="../../assets/Switchon.svg" @click="(getNotifications.ads_newsletter_sms_notification = 0),
          updateNotification()" v-if="getNotifications?.ads_newsletter_sms_notification" />
        <img src="../../assets/Switchoff.svg" @click="(getNotifications.ads_newsletter_sms_notification = 1),
          updateNotification()" v-else />
      </div>
    </div>
  </div>
...
<div class="payment-wrapper">
  <div class="payment-header">
    ...
    <on-boarding-button v-if="getUser?.account_type === 'super_admin'" @click="addAccount = true"
      fontsize="1rem" color="#FFFFFF" text-node="Add bank account" btn-width="12.25rem" height="2.5rem"
      border="none"></on-boarding-button>
  </div>
  ...
  <div>
    <div class="payment-choice">
      ...
      <p class="payment-m-s" :style="paymentType === 'bank'
        ? {
          borderBottomColor:
            custom_theme
              ? custom_theme.color
...
payment_type_active:
paymentType === 'bank',
}" style="cursor: pointer" @click="paymentType = 'bank'">
        Bank Account
      </p>
    </div>
    ...
    <div v-if="paymentType === 'bank'">
      <div v-if="getBankAccount?.length > 0">
        <div class="bank-wrapper">
          <div class="bank-card" v-for="(
                                          i, index
                                      ) in getBankAccount" :key="index">
            <div v-if="getUser?.account_type ===
              'super_admin'
              " @click="
showActions(index)
" style="
      ...
      <div v-else class="no-team-member">
        <svg xmlns="http://www.w3.org/2000/svg" width="116" height="116" viewBox="0 0 116 116" fill="none">
          <path
            d="M0.117188 57.883C0.117188 73.2346 6.21556 87.9574 17.0707 98.8125C27.9259 109.668 42.6487 115.766 58.0002 115.766C73.3518 115.766 88.0746 109.668 98.9297 98.8125C109.785 87.9574 115.883 73.2346 115.883 57.883C115.883 42.5315 109.785 27.8087 98.9297 16.9536C88.0746 6.09837 73.3518 0 58.0002 0C42.6487 0 27.9259 6.09837 17.0707 16.9536C6.21556 27.8087 0.117188 42.5315 0.117188 57.883Z"
            fill="#F1F2F6" />
          <path d="M8.3418 20.9214L67.758 3.4748L90.5149 80.9757L31.0986 98.4223L8.3418 20.9214Z"
            fill="white" stroke="#1D1E2C" stroke-width="1.79437" stroke-linecap="round"
            stroke-linejoin="round" />
          <path d="M20.0869 8.62842H82.0131V89.3999H20.0869V8.62842Z" fill="white" stroke="#1D1E2C"
            stroke-width="1.79437" stroke-linecap="round" stroke-linejoin="round" />
...
<header class="flex_between">
  <h3>Manage Markup</h3>
  <on-boarding-button @click="$refs.markupBuilder.openModal()" border="none" :plus_icon="true"
    btn-width="11.0625rem" text-node="Markup Builder"></on-boarding-button>
</header>
...
<header class="mb-0 flex_between">
  <div class="search-team">
    ...
    <input :disabled="!markupSearch &&
      !markup?.data?.length
      " @input="searchMarkup" id="markup_input_filter" v-model="markupSearch" type="search" style="
                              outline: none;
                              border: none;
                              width: 19.4rem;
...
<div class="table-wrapper">
  <domain-table @editMarkup="$refs.markupBuilder.openModal($event)" @cloneMarkup="$refs.markupBuilder.modalMainAction($event)" @deleteMarkup="$refs.markupBuilder.deleteMarkup($event)" :data="markup.data" :currentPage="markup.current_page" :itemsPerPage="markup.per_page"
    :lastPage="markup.last_page" :total="markup.total" :is-paginate="true" tableName="markup"
    :fields="markUpFields"></domain-table>
</div>
</div>
<!-- customization -->
...
<business-verification v-if="verification_Type === 'business'" :is-component="false"
  :in-route="false"></business-verification>
<upload-docs @parentDisableShow="" v-else :is-component="false" :in-route="false"
  v-if="verification_Type === 'docs'"></upload-docs>
</div></div>
</div>
</div>
</div>
...
doFilter() {
this.membersFilteredResult = this.getMembers.filter(
(it) =>
it.first_name?.toLowerCase() === this.searchQuery ||
it.last_name?.toLowerCase() === this.searchQuery ||
it.email?.toLowerCase() === this.searchQuery
);

if (this.membersFilteredResult.length > 0) this.noUser = false;
if (this.membersFilteredResult.length < 1) this.noUser = true;
if (this.searchQuery.length < 1) {
(this.membersFilteredResult = []), (this.noUser = false);
}
},
...
storeUtils
.fireAway()
.settings?.deleteBank(this.bankName.id)
.then(() => {
...
this.model.name = this.model.business_name
? this.model.business_address
: this.getBusinessProfile.name;
this.model.email = this.model.business_email
? this.model.business_email
: this.getBusinessProfile.email;
this.model.address = this.model.business_address
? this.model.business_address
: this.getBusinessProfile.address;
this.model.website = this.model.business_website
? this.model.business_website
: this.getBusinessProfile.website;
this.model.cac_number = this.model.cac_number
? this.model.cac_number
: this.getBusinessProfile.cac_number;
storeUtils.fireAway().auth?.commitErrors(this.error);
...
next()
  ...
  computed: {
    ...
    markup() {
      return storeUtils.fireAway().settings.markup;
    },
