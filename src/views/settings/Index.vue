...
export default {
    ...
    beforeRouteEnter(to, from, next) {
        next((vm) => {
            const userHash = vm.$route.hash.toLowerCase();
            if (userHash) {
                if (userHash == "#domain" || userHash == "#markup")
                    if (
                        vm.getUser?.account_type === "super_admin" ||
                        (vm.getUser?.account_type === "admin" &&
                            vm.getBusinessProfile?.is_cac_verified === "true" &&
                            vm.getBusinessProfile?.is_id_verified === "true")
                    ) {
                        next();
                    } else {
                        vm.$router.push("/");
