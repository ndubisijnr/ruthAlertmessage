          ...
          <div v-if="getCurrentRoute === 'Dashboard'">
            ...
            >
              <div class="with-tiqwa">
                <div>
                  <h3 class="with-tiqwa-h">
                    Hello,{{ getUser?.first_name }}, get started with Travel
                    Yakata 🎉
                  </h3>
                  <p class="with-tiqwa-p">
                    Please complete your setup to access Travel Yakata services,
                  </p>
                </div>
        ...
        <div class="travel_type_booking">
          ...
          <div
            ...
            v-if="
              ...
              getCurrentRoute !== 'Flight Payment'
            "
          >
            <div
              class="dest-abv"
              v-if="
                getFlightResult[0]?.outbound || getFlightResult[0]?.outbound
              "
            >
              <div class="dest-abv" v-if="getFlightResult[0]?.outbound">
                <p class="dest-abv-it">
                  {{
                    getCityByCityCode(
                      getFlightResult[0]?.outbound[0]?.airport_from
                    )
                  }}
                  ({{ getFlightResult[0]?.outbound[0]?.airport_from }})
                </p>
                ...
                <p class="dest-abv-it">
                  {{
                    getCityByCityCode(
                      getFlightResult[0]?.outbound.length > 0
                        ? getFlightResult[0]?.outbound[
                            getFlightResult[0]?.outbound.length - 1
                          ]?.airport_to
                        : getFlightResult[0]?.outbound[
                            getFlightResult[0]?.outbound.length - 1
                          ]?.airport_to
              ...
              <div class="booking-info" v-if="getFlightResult[0]?.outbound">
                <div class="search-info-area">
                  <p class="search-info">
                    {{
                      getFlightResult[0]?.inbound?.length > 0
                        ? "Round Trip"
                        : "One Way"
                    }}
                  </p>
                  ...
                  <p class="search-info">
                    {{ getFlightResult[0]?.outbound[0]?.cabin_type }}
                  </p>
            ...
            <div class="dest-abv" v-else>
              <div class="dest-abv">
                <p class="dest-abv-it">
                  {{
                    getCityByCityCode(
                      getFlightResult[0]?.routes[0].segments[0]?.airport_from
                    )
                  }}
                  ({{
                    getFlightResult[0]?.routes[0].segments[0]?.airport_from
                ...
                <p class="dest-abv-it">
                  {{
                    getCityByCityCode(
                      getFlightResult[0]?.routes[0].segments.length > 0
                        ? getFlightResult[0]?.routes[0].segments[
                            getFlightResult[0]?.routes[0].segments.length - 1
                          ]?.airport_from
                        : getFlightResult[0]?.routes[0].segments[
                            getFlightResult[0]?.routes[0].segments.length - 1
                          ]?.airport_from
              ...
              <div class="booking-info">
                <div class="search-info-area">
                  ...
                  <p class="search-info">
                    {{ getFlightResult[0]?.routes[0].segments[0].cabin_type }}
                  </p>
                  <!-- <p class="search-info">{{ getFlightResult[0]?.outbound[0]?.cabin_type }}</p> -->
                </div>
              </div>
            ...
            <on-boarding-button
              ...
              :background="
                custom_theme
                  ...
                  : lightenColor(default_theme.color)
              "
              :text-node="edit ? 'Cancel' : 'Edit Search'"
            ></on-boarding-button>
          </div>
        </div>
        <br />
            ...
            <p
              ...
              :style="
                activeDestType === 'round_trip'
                  ...
                  : {}
              "
              @click="activeDestType = 'round_trip'"
              :class="{
                activeDestType: activeDestType === 'round_trip',
              }"
            >
              Round Trip
            </p>
            ...
            <p
              ...
              :style="
                activeDestType === 'one_way'
                  ...
                  : {}
              "
              @click="activeDestType = 'one_way'"
              :class="{
                activeDestType: activeDestType === 'one_way',
              }"
            >
              One Way
            </p>
            ...
            <p
              ...
              :style="
                activeDestType === 'multiCity'
                  ...
                  : {}
              "
              @click="activeDestType = 'multiCity'"
              :class="{
                activeDestType: activeDestType === 'multiCity',
              }"
            >
              Multi City
            </p>
          ...
          >
            <div style="width: 100%">
              <on-boarding-input
                ...
                autocomplete="off"
                width="100%"
                id="search_model_from_input"
                class=""
                @inputValue="
                  (value) => {
                    (this.fromQuery = value), filterAirportFrom();
                  }
                "
              ></on-boarding-input>
              <div
                class="airportsDropDown"
                v-if="this.filteredAirportFrom.length > 0"
              >
                <p
                  @click="
                    selectDestination(
                      'search_model_from_input',
                      `${i.city} - ${i.name}`,
                      `${i.iata_code}`
                    )
                  "
                  class="per_airport"
                  v-for="(i, index) in filteredAirportFrom"
                  :key="index"
                >
                  {{ i.city }} - {{ i.country }} -
                  {{ i.name }}
            ...
            <div style="width: 100%">
              <on-boarding-input
                label="To"
                is-fake-loading="true"
                id="search_model_to_input"
                class=""
                @inputValue="
                  (value) => {
                    (this.toQuery = value), filterAirportTo();
                  }
                "
              ></on-boarding-input>
              <div
                class="airportsDropDown"
                v-if="this.filteredAirportTo.length > 0"
              >
                <p
                  @click="
                    selectDestination(
                      'search_model_to_input',
                      `${i.city} - ${i.name}`,
                      `${i.iata_code}`
                    )
                  "
                  class="per_airport"
                  v-for="(i, index) in filteredAirportTo"
                  :key="index"
                >
                  {{ i.city }} - {{ i.country }} -
                  {{ i.name }}
            ...
            <DataPicker
              ...
              v-if="activeDestType === 'round_trip'"
                ...
                {{
                  flightModel.children > 0
                    ? `${flightModel.children} ${
                        flightModel.children > 1 ? "Children" : "child"
                      } `
                    : null
                }}
              </p>
              <div v-if="showPassengers" class="dropDown">
                <div class="doc_type_options">
                  <div class="passenger-type">
                    ...
                    >
                      <button
                        :disabled="flightModel.adults < 1"
                        @click="passengerSelectionControl('adult', 'minus')"
                        class="minus-button"
                      >
                        -
                      </button>
                      ...
                      <button
                        :disabled="passenger_disable_buttons"
                        @click="passengerSelectionControl('adult', 'add')"
                        class="add-button"
                      >
                        +
                      </button>
                  ...
                  <div class="passenger-type">
                    ...
                    >
                      <button
                        :disabled="flightModel.children < 1"
                        @click="passengerSelectionControl('children', 'minus')"
                        class="minus-button"
                      >
                        -
                      </button>
                      ...
                      <button
                        :disabled="passenger_disable_buttons"
                        @click="passengerSelectionControl('children', 'add')"
                        class="add-button"
                      >
                        +
                      </button>
                  ...
                  <div class="passenger-type">
                    ...
                    >
                      <button
                        :disabled="flightModel.infants < 1"
                        @click="passengerSelectionControl('infants', 'minus')"
                        class="minus-button"
                      >
                        -
                      </button>
                      ...
                      <button
                        :disabled="passenger_disable_buttons || infant_disable"
                        @click="passengerSelectionControl('infants', 'add')"
                        class="add-button"
                      >
                        +
                      </button>
                ...
                <div class="doc_type_options">
                  <div class="passenger-type" style="border: none">
                    <p
                      ...
                      @click="
                        (flightModel.cabin = 'economy'),
                          (showClass = !showClass)
                      "
                    >
                      Economy
                      <svg
                        v-if="flightModel.cabin === 'economy'"
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                  ...
                  <div class="passenger-type" style="border: none">
                    <p
                      class="passenger-type-text-1"
                      @click="
                        (flightModel.cabin = 'premium_economy'),
                          (showClass = !showClass)
                      "
                    >
                      Premium Economy
                      <svg
                        v-if="flightModel.cabin === 'premium_economy'"
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                  ...
                  <div class="passenger-type" style="border: none">
                    <p
                      class="passenger-type-text-1"
                      @click="
                        (flightModel.cabin = 'business'),
                          (showClass = !showClass)
                      "
                    >
                      Business Class
                      <svg
                        v-if="flightModel.cabin === 'business'"
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
                  ...
                  <div class="passenger-type" style="border: none">
                    <p
                      class="passenger-type-text-1"
                      @click="
                        (flightModel.cabin = 'first'), (showClass = !showClass)
                      "
                    >
                      First Class
                      <svg
                        v-if="flightModel.cabin === 'first'"
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
                        viewBox="0 0 16 16"
          ...
          <div v-else>
            <div style="gap: 0.5rem" class="d-flex flex-column">
              ...
              <div class="d-flex gap-3 align-center">
                <div class="choose_document_type" style="position: relative">
                  ...
                  <p class="selected-item">
                    ...
                    {{
                      flightModel.infants > 0 && flightModel.adults > 0
                        ? ","
                        : null
                    }}
                    {{
                      ...
                      <div class="passenger-type">
                        ...
                        >
                          <button
                            :disabled="flightModel.children < 1"
                            @click="
                              passengerSelectionControl('children', 'minus')
                            "
                            class="minus-button"
                          >
                            -
                          ...
                          <button
                            :disabled="passenger_disable_buttons"
                            @click="
                              passengerSelectionControl('children', 'add')
                            "
                            class="add-button"
                          >
                            +
                      ...
                      <div class="passenger-type">
                        ...
                        >
                          <button
                            :disabled="flightModel.infants < 1"
                            @click="
                              passengerSelectionControl('infants', 'minus')
                            "
                            class="minus-button"
                          >
                            -
                          ...
                          <button
                            :disabled="
                              passenger_disable_buttons || infant_disable
                            "
                            @click="passengerSelectionControl('infants', 'add')"
                            class="add-button"
                          >
              ...
              <on-boarding-button
                ...
                text-node="Search"
                @click="reSearch"
                :disabled="getLoading || disableBtn().val"
                :loading="getLoading"
              ></on-boarding-button>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    openDrop(value) {
      ...
      if (value === "class") {
        (this.showClass = !this.showClass), (this.showPassengers = false);
        if (this.showClass) editSearchDiv.classList.add("additional");
        else editSearchDiv.classList.remove("additional");
      } else {
        (this.showPassengers = !this.showPassengers), (this.showClass = false);
    ...
    filterAirportFrom() {
      if (this.fromQuery.length < 1) {
        ...
        this.filteredAirportFrom = this.getAirports.filter((it) => {
          ...
          return (
            it.city_code === this.fromQuery.toUpperCase() ||
      ...
      if (this.toQuery.length < 1) {
        ...
        this.filteredAirportTo = this.getAirports.filter((it) => {
          return (
            ...
            it.city_code === this.toQuery.toUpperCase() ||
    ...
    editSearch() {
      ...
      if (this.edit) {
        ...
        editSearchDiv.classList.remove(
    ...
    getCityByCityCode(city_code) {
      ...
      if (airports) {
        const cityName = airports.filter((it) => it.iata_code === city_code)[0]
          ?.city;
        return cityName;
      }
    },
